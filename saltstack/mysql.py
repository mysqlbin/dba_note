...
Required python modules: pymysql
...
def _get_serv(ret=None, commit=False):
    ...
    if __context__ and 'mysql_returner_conn' in __context__:
        try:
            ...
            conn.ping()
            connect = False
        except pymysql.connections.OperationalError as exc:
            log.debug('OperationalError on ping: %s', exc)

    if connect:
        log.debug('Generating new MySQL connection pool')
        try:
            # An empty ssl_options dictionary passed to pymysql.connect will
            ...
            conn = pymysql.connect(host=_options.get('host'),
