...
2. checkpoint	
	...
	MongoDB 也有checkpoint，当把脏页刷盘，并做一个标记点即checkpoint，对应的journal日志就可以进行清除
		...
		-- 存储引擎的checkpoint检测点，实际上就是做快照，把当前存储引擎的脏数据全部记录到磁盘。
		...
		journal日志默认每个100毫秒同步磁盘一次，每100M数据生成一个新的journal文件同时会把日志同步到磁盘，journal默认使用了snappy压缩，检测点创建后，此前的journal日志即可清除。
			-- 检测点创建后，此前的journal日志即可清除。这里描述不对，应该改为检查点之前的journal日志就可以不需要维护了。
		...
		在Checkpoint操作开始时，WiredTiger提供指定时间点（point-in-time）的数据库快照（Snapshot），该Snapshot呈现的是内存中数据的一致性视图。 -- 重点之一。
		当向Disk写入数据时，WiredTiger将Snapshot中的所有数据以一致性方式写入到数据文件（Disk Files）中。
		一旦Checkpoint创建成功，WiredTiger保证数据文件和内存数据是一致性的，因此，Checkpoint相当于还原点（Recovery Point），Checkpoint操作能够缩短MongoDB从Journal日志文件还原数据的时间。
...
4. 相关思考
	...
	checkpoint、脏数据页、journal 之间的关系
	checkpoint跟后台evict page线程（threads_min、threads_max）的关系
	
		
5. 小结
	checkpoint的作用表现为释放了内存空间和加快数据库的恢复速度：
		每隔60秒触发一次checkpoint，把脏页进行刷盘并做一个标记点，标记点之前的内存数据跟磁盘上的一致，并且该标记点对应的journal日志就可以不需要维护了。
		同时加快了数据库的恢复速度：Checkpoint相当于还原点（Recovery Point），作为崩溃恢复的起点。
