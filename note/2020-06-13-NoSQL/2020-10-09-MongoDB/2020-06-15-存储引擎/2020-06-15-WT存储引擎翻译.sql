...
3. Journal
	...
	WiredTiger日记保留检查点之间的所有数据修改。 如果MongoDB在检查点之间退出，它将使用日志重放自上一个检查点以来修改的所有数据。 有关MongoDB将日记数据写入磁盘的频率的信息，请参阅日记处理。
	...
	您可以通过将storage.journal.enabled设置为false来禁用独立实例的日志记录，这样可以减少维护日志记录的开销。 
	
	对于独立实例，不使用日志意味着MongoDB意外退出时，您将丢失最后一个检查点之前的所有数据修改。  -- 单实例下也要开启 journal.
...
4. 压缩 --Compression
	...
	默认情况下，WiredTiger journal日志也被压缩。 有关日记压缩的信息，请参阅日记。
...
5. 内存使用 --Memory Use
	...
	WiredTiger内部缓存中的数据与磁盘格式使用不同的表示形式：
		...
		加载到WiredTiger内部缓存中的索引具有与磁盘格式不同的数据表示形式，但仍可以利用索引前缀压缩来减少RAM使用。 
		索引前缀压缩对来自索引字段的通用前缀进行重复数据删除。
		WiredTiger内部缓存中的收集数据未压缩，并且使用与磁盘格式不同的表示形式。 
