...
0. 旧版本
	...
	对指定集合构建索引时会独占该集合的读写锁（注：构建索引时会锁表）。
	对集合构建索引时该集合读和写操作会加锁，直到mongod进程完成索引构建，该集合的读写操作锁才会释放。
	
	--------------------------------------------------------------------------------------------------

	总的来说，就是会锁集合，阻塞对集合的读和写请求，直到创建索引完成。
...
2. 加锁过程
	...
	2. 释放集合的排他锁，对集合的数据加意向排他锁(类似于MDL读锁)，可以做DML操作
	
	3. 扫描集合的数据完成后，释放意向排他锁，添加排他锁，此时会阻塞集合的读写操作
