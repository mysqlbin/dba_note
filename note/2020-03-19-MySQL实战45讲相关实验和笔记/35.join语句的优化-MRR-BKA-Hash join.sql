...
6. 小结
	...
	MRR 是多范围读取的意思, 先按照满足条件的二级索引中的主键ID进行升序排序, 然后再根据主键ID进行有序的回表查询数据
	然后得到一个结果集放到 join buffer中
	再通过 BKA优化 把 join buffer 中数据批量跟被驱动表的列做关联查询.
	
	-- 理解了.
	
	2.4 MRR能够提升性能的核心：
		这条查询语句在索引 a 上做的是一个范围查询（也就是说，这是一个多值查询），可以得到足够多的主键 id
		这样通过主键ID排序以后，再去主键索引查数据，才能体现出 顺序性 的优势。
