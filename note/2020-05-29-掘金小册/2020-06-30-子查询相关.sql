...
2.1 物化表的提出
	...
	1. MATERIALIZED：表示物化表，物化表用于存储子查询结果集的列
	
	2. 如果单独执行子查询后的结果集太多的话，就会导致这些问题：
...
2.6 如何优化不能转为semi-join查询的子查询
	...
	1. 对于不相关子查询来说，可以尝试把它们物化之后再参与查询
		...
		案例一: 参考笔记 《2020-06-30-explain.sql》 中的《5.5 SUBQUERY 》
...
2.7 小结
	...
	3. 主要知识点   -- 个人的总结和理解
		针对in子查询存在的性能问题, 然后MySQL内部通过物化表和semi join来做优化 即 物化表和 semi join 都是作为in子查询的MySQL内部优化手段。
		使用到 semi join 一定会用到物化表暂存数据，使用到物化表暂存数据并不一定会用到semi join。
		然后in子查询转 semi join 还是有一定限制的.	
		
		物化表
			是什么：执行in子句产生的结果集写入临时表中，这个临时表在内部称为物化表。
			作用：  用来暂存执行in子句返回的数据，从而提升in子查询的性能。
			限制：相关子查询并不是一个独立的查询，所以不能转换为物化表来执行查询，如下SQL语句：
				EXPLAIN SELECT * FROM t1 WHERE key1 IN (SELECT key1 FROM t2 WHERE t1.key2 = t2.key2) OR key3 = '3';
			
		semi join
			是什么：即半连接，将会利用子查询部分作为构建物化表，将in子查询转换半连接的的方式进来匹配结果
			作用：子查询转换为半连接可以充分发挥优化器的作用
			限制：参考 2.5 不适用于semi-join的情况

		semi join 跟 inner join的区别：
			inner join 用的全连接，假设驱动表的列在被驱动表中匹配到2条记录，那么就会把这2条记录加入到结果集中。
			semi join 用的是半连接，驱动表的列在被驱动表中最多只会匹配到1条记录，因为被驱动表的结果集已经去重。
			这就是两者的差别。
