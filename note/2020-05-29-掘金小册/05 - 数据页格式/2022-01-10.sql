...
页目录(Page Directory)
	...
	槽(Slot)
		页目录中的这些地址偏移量被称为槽（英文名：Slot），所以这个页面目录就是由槽组成的。

	页目录：每个组最后一条记录的地址偏移量，单独提取出来按顺序存储到靠近页的尾部的地方
	槽：    页目录中 每个组最后一条记录的地址偏移量
...
3. 页目录(Page Directory)			
	...
	所以分组是按照下边的步骤进行的：
>>>>>>> 29946c6e3120284af21427803910b8c4aa10600b
...
<<<<<<< HEAD
	...
	所以分组是按照下边的步骤进行的：
		...
		2. 之后每插入一条记录，都会从页目录中找到主键值比本记录的主键值大并且差值最小的槽，  -- 这里还不理解。
