...
1.1 连接的本质 
	...
	两个表的连接查询
		root@localhost [db3]>select * from t1,t2;
		+------+------+------+------+
		| m1   | n1   | m2   | n2   |
		+------+------+------+------+
		|    1 | a    |    2 | b    |
	...
	笛卡尔积:   两个表或者两个表以上进行连接查询得到的结果集, 称为 笛卡尔积.
...
1.2 连接过程简介
	...
	那么这个连接查询的大致执行过程如下：
		1. 首先确定第一个需要查询的表， 这个表称之为驱动表
			...
			此时, t1表中符合t1.m1 > 1的记录有两条:
				root@localhost [db3]>select * from t1 where m1>1;
				+------+------+
				| m1   | n1   |
				+------+------+
				|    2 | b    |
		...
		2. 到被驱动表匹配记录
			...
			上一步骤从驱动表中得到了2条记录，所以需要查询2次t2表。此时涉及两个表的列的过滤条件t1.m1 = t2.m2就派上用场了：
				2.1 当t1.m1 = 2时，过滤条件t1.m1 = t2.m2就相当于t2.m2 = 2，所以此时t2表相当于有了t2.m2 = 2、t2.n2 < 'd'这两个过滤条件，然后到t2表中执行单表查询。
					
					root@localhost [db3]>select * from t2 where t2.m2 = 2 and t2.n2 < 'd';
					+------+------+
					| m2   | n2   |
					+------+------+
					|    2 | b    |
				...
				2.2 当t1.m1 = 3时，过滤条件t1.m1 = t2.m2就相当于t2.m2 = 3，所以此时t2表相当于有了t2.m2 = 3、t2.n2 < 'd'这两个过滤条件，然后到t2表中执行单表查询。
					...
					root@localhost [db3]>select * from t2 where t2.m2 = 3 and t2.n2 < 'd';
					+------+------+
					| m2   | n2   |
					+------+------+
					|    3 | c    |
			...
			也就是说整个连接查询最后的结果只有两条符合过滤条件的记录：
				...
				root@localhost [db3]>SELECT * FROM t1, t2 WHERE t1.m1 > 1 AND t1.m1 = t2.m2 AND t2.n2 < 'd';
				+------+------+------+------+
				| m1   | n1   | m2   | n2   |
				+------+------+------+------+
				|    2 | b    |    2 | b    |
	...
	如果我们把t1.m1 > 1这个条件去掉，那么从t1表中查出的记录就有3条，就需要查询3次t2表了。也就是说在两表连接查询中，驱动表只需要访问一次，被驱动表可能被访问多次。
...
2.1 表结构和数据初始化
	...
	root@localhost [db3]>select * from student;
	+----------+-----------+--------------------------+
	| number   | name      | major                    |
	+----------+-----------+--------------------------+
	| 20180101 | 杜子腾    | 软件学院                 |
	...
	root@localhost [db3]>select * from score;
	+----------+-----------------------------+-------+
...
2.2 左(外)连接的语法
	...
	把所有的学生的成绩信息都查询出来，即使是缺考的考生也应该被放到结果集中：		
		...
		root@localhost [db3]>SELECT s1.number, s1.name, s2.subject, s2.score FROM student as s1 left join score as s2 on s1.number = s2.number;
		+----------+-----------+-----------------------------+-------+
		| number   | name      | subject                     | score |
		+----------+-----------+-----------------------------+-------+
		| 20180101 | 杜子腾    | 母猪的产后护理              |    80 |
...
2.5 小结
	...
	而对于内连接来说，由于凡是不符合ON子句或WHERE子句中的条件的记录都会被过滤掉，其实也就相当于从两表连接的笛卡尔积中把不符合过滤条件的记录给踢出去，所以对于内连接来说，驱动表和被驱动表是可以互换的，并不会影响最后的查询结果。
