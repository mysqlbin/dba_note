...
3. 连接的原理
	...
	3.1 嵌套循环连接(Nested-Loop Join)
		...
		使用NLJ算法分析语句的执行流程：	
			...
			语句：
				...
				mysql> desc  SELECT * FROM t1, t2 WHERE t1.m1 > 1 AND t1.m1 = t2.m2 AND t2.n2 < 'd';
				+----+-------------+-------+------------+------+---------------+------+---------+------+------+----------+----------------------------------------------------+
			...
			有索引的执行流程：
				...
				t2.m2 是普通索引
					...
					alter table t2 add index idx_m2(`m2`);
					...
					+----+-------------+-------+------------+------+---------------+--------+---------+-----------------+------+----------+-------------+
					|  1 | SIMPLE      | t1    | NULL       | ALL  | NULL          | NULL   | NULL    | NULL            |    3 |    33.33 | Using where |
					|  1 | SIMPLE      | t2    | NULL       | ref  | idx_m2        | idx_m2 | 5       | mysqlslap.t1.m1 |    1 |    33.33 | Using where |
					+----+-------------+-------+------------+------+---------------+--------+---------+-----------------+------+----------+-------------+
					2 rows in set, 1 warning (0.00 sec)

					type = ref：基于二级索引的等值查询。
				
				t2.m2 是普通唯一索引
					
					t2.m2可以为NULL 
						alter table t2 add unique index idx_m2(`m2`), drop index idx_m2;
					...
					t2.m2不可以为NULL
						...
						+----+-------------+-------+------------+--------+---------------+--------+---------+-----------------+------+----------+-------------+
						|  1 | SIMPLE      | t1    | NULL       | ALL    | NULL          | NULL   | NULL    | NULL            |    3 |    33.33 | Using where |
						|  1 | SIMPLE      | t2    | NULL       | eq_ref | idx_m2        | idx_m2 | 4       | mysqlslap.t1.m1 |    1 |    33.33 | Using where |
						+----+-------------+-------+------------+--------+---------------+--------+---------+-----------------+------+----------+-------------+
						2 rows in set, 1 warning (0.00 sec)
				...
				假设m2列是t2表的主键或者唯一二级索引列，那么使用t2.m2 = 常数值这样的条件从t2表中查找记录的过程的代价就是常数级别的。
	...
	3.2 基于块的嵌套循环连接(Block Nested-Loop Join)
		...
		join buffer就是执行连接查询前申请的一块固定大小的内存，先把若干条驱动表结果集中的记录装在这个join buffer中，然后开始扫描被驱动表，
		每一条被驱动表的记录一次性和join buffer中的多条驱动表记录做匹配，因为匹配的过程都是在内存中完成的，所以这样可以显著减少被驱动表的I/O代价。
		
		
		最好的情况是join buffer足够大，能容纳驱动表结果集中的所有记录，这样只需要访问一次被驱动表就可以完成连接操作了
		
		驱动表的记录并不是所有列都会被放到join buffer中，只有查询列表中的列和过滤条件中的列才会被放到join buffer中，所以再次提醒我们，最好不要把*作为查询列表，只需要把我们关心的列放到查询列表就好了，这样还可以在join buffer中放置更多的记录呢哈。
			-- 查询语句，只取必要的字段。
