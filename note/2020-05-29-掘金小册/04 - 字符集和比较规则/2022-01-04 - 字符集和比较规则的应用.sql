...
1.3 一些重要的字符集
	...
	ASCII字符集
		...
		由于总共才128个字符，所以可以使用1个字节来进行编码，我们看一些字符的编码方式：

		'L' ->  01001100（十六进制：0x4C，十进制：76）
		'M' ->  01001101（十六进制：0x4D，十进制：77）
		
		
	latin1(ISO 8859-1)字符集
...
3.2 服务器级别的字符集和比较规则
	...
	参数character-set-server = utf8/utf8mb4的系统变量
		...
		2. character-set-server = utf8mb4
			...
			| character_set_connection | utf8mb4                                                        |    -- 服务器处理请求时会把请求字符串从 character_set_client 转为 character_set_connection
			| character_set_database   | utf8mb4                                                        |    -- 当前数据库的字符集
...
4. 编码和解码使用的字符集不一致的后果
	...
	对于同一个字符串编码和解码使用的字符集不一样，会产生意想不到的结果，作为人类的我们看上去就像是产生了 乱码 一样。
...
5. MySQL中字符集的转换
	...
	从发送请求到接收结果过程中发生的字符集转换：
		...
		4. 将从某个列获取到的字节串从该列使用的字符集转换为 character_set_results 代表的字符集后发送到客户端。
			...
			假设你的客户端采用的字符集和 character_set_results 不一样的话，这就可能会出现客户端无法解码结果集的情况，结果就是在你的屏幕上出现 乱码 。
			比如我的客户端使用的是utf8字符集，如果把系统变量character_set_results的值设置为ascii的话，可能会产生 乱码 。
