...
2. 可见性判断规则
   ...
   数据版本的可见性规则，就是基于数据的 `DATA_TRX_ID` (行记录的事务ID) 和这个一致性视图里面的活跃事务ID列表(`m_ids`)对比结果得到的，如下：
       ...
       2. 处于 `m_ids` 的有两种情况：
          ...
          不在活跃事务列表中， 在 m_ids 范围内， 表示这个版本是已经提交了的事务生成的，可见。 # 理解，需要例子来验证下：参考笔记 《8-RR和RC隔离级别的实现.sql》 中的 2. RC隔离级别的实现      
...
4. 通过人肉分析可见性规则
	...
	4.1. 事务 A 此时的视图数组:
		...
		1.3 read view:
			事务 A 的 S1时刻: [null]，当前系统已经创建过的最大事务ID为1， 所以高水位为 2
			事务 A 的 S2时刻: [null]，使用的是 S1时刻 的视图，高水位为 2
			事务 A 的 S3时刻: [null]， 同样使用的是 S1时刻 的视图，高水位为 2
	...
	4.3. 事务 A 的S3时刻的查询结果为 (1,1,'a') 分析
		...
		2. 事务 A 的S3时刻的查询结果为 (1,1,'a') 分析:
			...
			再往前找, 通过 rool_ptr 构建出 (1,1,'a'), 它的 row trx_id = 1, 比高水位(m_up_limit_id=2)小, 可见.
			所以最后返回(1,1,’a’)
