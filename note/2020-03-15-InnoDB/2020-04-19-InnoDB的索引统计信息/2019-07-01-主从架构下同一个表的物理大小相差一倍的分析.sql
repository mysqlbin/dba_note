...
2. 现象
3. 统计表的大小
4. 主从服务器上，同一个表的表空间文件大小相差特别大，可能的原因
5. 初步结论
6. 分析过程如下
	6.1 主从表结构一致
	6.2 主从表的行记录格式都是Dynamic
	6.3 查看索引的状态信息和统计数据表的总行数
	6.4 查看表的统计信息
	6.5 查看索引的统计信息	
7. alter+analyze+optimize
8. 从库上重建表
9. 小结
...
2. 现象
	...
	主库单表的大小比从库的大一倍；
	从库查询该表的数据出现慢查询，但是把查询拿到主库上执行没有出现慢查询
...
4. 主从服务器上，同一个表的表空间文件大小相差特别大，可能的原因
	...
	6. 索引统计信息不准确/索引统计信息不够新    --这个是根据自己的经验加上的。
		
	
5. 初步结论
	从 information_schema.tables的信息中可以看到:
		主库和从库的碎片大小分别为 7MB和5MB,  排除了 1.碎片不同会导致表空间相关很大.
		主库的 table_rows 比从库的 table_rows 大了一倍, 根据以往的经验, 初步估计是 表或者索引统计信息不准确/索引统计信息不够新 造成此案例中的现象.
...
6. 分析过程如下
	...
	6.3 查看索引的状态信息和统计数据表的总行数
		...
		统计数据表总行数的SQL先在主库执行, 时间大概相隔一秒之后在从库执行, 根据主从复制的原理, 这里看到的从库的记录数比主库的记录数多10行, 是属于正常现在的; 
...
9. 小结
	...
	主从服务器上，同一个表的表空间文件大小相差特别大，可能的原因如下：
		...
		4. 主从表的行格式不一致，如主库为dynamic，从库为compressed
		5. 个别云数据库在从库上可能采用特殊的并行复制技术，导致在从库上有更高的碎片率（有个极端的案例，同一个表在主库只有6G，从库上则有将近150G）
		6. 表或者索引统计信息不准确/索引统计信息不够新    --这个是根据自己的经验加上的。
	
	解决办法：
		analyze table 重新统计索引的统计信息
		不需要重建表
	
	索引统计信息不准确的影响：
		SQL语句使用了错误的索引，导致查询速度变慢，对业务有一定的影响
	
	相关现象：
		1. 如果你发现 explain 的结果预估的 rows 值跟实际情况差距比较大
		2. 部分SQL语句执行速度慢
