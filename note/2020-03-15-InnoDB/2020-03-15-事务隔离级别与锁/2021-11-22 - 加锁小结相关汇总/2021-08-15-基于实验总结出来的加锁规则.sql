...
3. 小结
	...
	1. RC隔离级别锁的范围更小、锁的时间更短。
		范围小：只有行锁。
		锁的时间短：在语句执行结束，不需要等事务提交，就会把不符合条件的记录锁释放掉。
		
	2. 使用主键索引作为数据更新和删除的条件是一个好的选择。
		

	
	RR级别：扫描到的数据都会加行锁和间隙锁 
	RC级别：扫描到的数据都会加行锁，但是不满足条件的数据，没有到commit阶段，就会释放，违反了两阶段加锁原则
	
	全表扫描指的是扫描主键索引的所有数据。

	
4. 参考实验
	2021-08-04-一些加锁规则的验证	
