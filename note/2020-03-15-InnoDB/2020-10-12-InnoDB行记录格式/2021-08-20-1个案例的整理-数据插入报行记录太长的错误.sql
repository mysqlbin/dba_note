...
2. 多个 blob 大字段发生了行溢出，导致实际存储到数据库的行记录长度大于8126
	...
	2.1 现象
		...
		-- 问题和原因
			刚进入公司2个多月，那时还没有DBA, 某天的下午5点多业务日志报数据插入失败，多个 blob 大字段发生了行溢出，导致行内保留的字节数大于8126，产生SQL语句插入失败的问题。
	...
	2.2 分析
		...
		行记录格式为compact
			...
			-- 对于行溢出的字段，会把数据的前768个节点+20个字节的指针 存储到行记录中，剩余的数据则会存储在大对象页中
	...
	2.3 解决办法
		...
		2. 修改表的行记录格式需要重建表，不是很紧急的修改要在业务低峰期执行
			...
			2.3 凌晨5点起来把所有的表的行记录格式都改为 dynamic，也是先修改从库的，再修改主库的。
