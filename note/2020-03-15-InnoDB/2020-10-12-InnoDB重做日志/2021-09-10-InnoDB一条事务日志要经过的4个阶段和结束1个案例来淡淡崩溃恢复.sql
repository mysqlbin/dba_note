...
2. 崩溃恢复的场景
	...
	有 1-10个事务已经写入 redo log buffer    			-- 创建日志阶段，记为LSN1
	...
	如果这个时候发生 crash recovery，那么在恢复的时候，1-7 的第7个事务事务作为崩溃的起点，然后顺序的扫描redo，大概过程如下：
		...
		LSN2 - LSN4 = 表示崩溃后实际要恢复的日志量, 也就是需要应用第9个事务的redo日志+对应磁盘上的数据页进行恢复出1个最新更新的数据页出来;
			-- 因为 redo 已经提交，所以可以应用redo恢复出1个最新更新的数据页出来;
			-- crash-safe的工作流程。
		...
		第10个事务的处理会相对比较复杂：
			  ...
			  2. binlog的完整性
				  通过binlog-checksum参数验证binlog的完整性，如果binlog是完整的，就提交事务
				  否则，回滚事务。
	
	checkpoint在崩溃恢复中的作用：
	
		只需要判断 第9和第10 这2个事务需要回滚 还是 刷盘， 在崩溃恢复的场景下大大缩短了恢复时间。
		
3. 小结
	1个案例，就把崩溃恢复涉及到知识点(redo log、2阶段提交)串联起来了。
