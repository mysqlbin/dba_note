...
2.2 通过 space-indexes 查看索引信息
	...
	275         PRIMARY                         3           internal    1           1           100.00%     
	275         PRIMARY                         3           leaf        3           3           100.00%     
	276         idx_name                        4           internal    1           1           100.00%     
	276         idx_name                        4           leaf        0           0           0.00%  
...
2.3 通过 index-recurse 查看主键的递归索引
	[root@env27 data]#innodb_space -s ibdata1 -T zst/t_20200403 -I PRIMARY index-recurse  > 1.sql
	
	[root@env27 data]# grep "LEAF" 1.sql  | tail -5
...
2.4 查看数据表的.ibd 文件的大小
	...
	.ibd 文件的大小
		...
		-rw-r----- 1 mysql mysql 147456 Mar 30 00:29 zst/t_20200403.ibd
			
		可以看到， .ibd 数据文件的大小为 147456 字节
		

2.5 删除一个数据页(page no = 5)的所有记录
	select table_schema,table_name,DATA_FREE,CREATE_TIME, UPDATE_TIME from  information_schema.TABLES where table_schema='zst' and table_name='t_20200403';
	...
	+--------------+------------+-----------+---------------------+---------------------+
	| zst          | t_20200403 |         0 | 2020-03-30 00:29:41 | 2020-03-30 00:29:52 |
	+--------------+------------+-----------+---------------------+---------------------+
	1 row in set (0.00 sec)
	
	
	root@localhost [zst]>delete from zst.t_20200403 where id <=276;
	Query OK, 276 rows affected (0.00 sec)
	
	查看 .ibd 文件物理数据大小	
		[root@env27 data]# ll zst/t_20200403.ibd
		-rw-r----- 1 mysql mysql 163840 Mar 30 00:43 zst/t_20200403.ibd
