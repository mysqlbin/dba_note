...
1. binlog cache 简介
	整个事务的 event 在 事务 commit 的时候才会真正写入 binlog log ， 在些之前这些 event 都存放在另外一个地方我们可以统称为 binlog cache。
	参数 binlog_cache_size:
		用于控制单个线程内 binlog cache 所占内存的大小
		默认大小为 32768 bytes 
		会话级别的参数，如果开启了 binary log 功能，则在事务期间用于保存 event 的缓存大小
		如果 经常使用大事务应该加大这个参数，避免过多的物理磁盘使用
	
		注意：
			加大 binlog_cache_size 来减少临时文件的产生，但是这样会增加内存消耗，试想以下，假如 binlog_cache_size 增加到 32MB，当有 300 个数据库连接 时，每个连接都会分配 32MB 的 binlog_cache( 不管你用多少)，那么就是将近 10G，很容易导致内存溢出，被系统 OOM。
...
2. 使用 binlog cache 的大概流程
	...
	2. 事务执行过程中
		...
		2.3 如果 binlog cache 缓冲区已经写满了，则将 binlog cache 缓冲区的数据写入到 binlog cache 临时文件，同时清空 binlog cache
			这个临时文件名以 ML 开头
