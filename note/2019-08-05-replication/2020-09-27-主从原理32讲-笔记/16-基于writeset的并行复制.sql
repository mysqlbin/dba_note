...
5. writeset的并行复制对降低 last commit 的处理流程
	...
	对降低 last commit 的处理流程
		...
		5. 第五步 row10.hashval 在 writeset 历史 MAP 中查找，没有找到冲突的行，需要做以下操作：
			将这一行插入到 writeset 历史MAP 中查找
			   （需要判断是否导致 历史MAP 占满， 如果占满则不需要插入，后面随即要清理掉，然后将本事务的 seq number = 130 写入 m_writeset_history_start, 作为最早的 seq number ）
			将 row10.hashval 和 seq number = 130 插入到 writeset 历史MAP 中。
			
		6. 至此，整个过程结束。可以看到，last commit 由以前的 125 降低为 120， 目的达到了。
			并且可以看出 writeset的历史MAP 相当于保存了一段时间以来修改行的快照，如果保证本次事务修改的数据在这段时间内没有冲突，那么显然是可以在从库并行执行的。
...
10. 小结
	...
	writeset是先根据commit_order分配last_committed和sequence_number，然后在根据writeset hash表进行冲突检测，再确定last_commit的值？
		...
		没有冲突：就设置为 m_writeset_history_start 的值，writeset 的历史 MAP 中最早事务(最老)的 sequence_number

		有冲突：  最新一次本行数据修改事务的 sequence_number 用来赋值给 last_committed
