...
2. 从库SQL线程的流程解析
	...
	2.8 sql_slave_skip_counter 参数 
		...
		虽然主从复制出现的故障成功跳过了，但只是暂时恢复了正常的主从复制状态，需要尽快的对Slave缺失的数据进行补齐，不然Master对Slave不存在的数据做的变更，仍然会重复导致主从复制故障，
		笔者觉得如果你的数据量差异不是太大的话，可以考虑使用pt-table-checksum和pt-table-sync工具进行恢复，如果你的数据量很大且数据差异很多，还是建议重做Slave较好，因为使用工具会锁表，会对线上业务造成一定的影响
