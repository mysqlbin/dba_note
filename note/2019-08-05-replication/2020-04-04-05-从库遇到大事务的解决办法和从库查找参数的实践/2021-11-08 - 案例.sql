...
1. 描述
	...
	现象：
		...
		3. 通过监控发现问题：
			...
			主库：
				有长事务，是1个删除全表记录的delete语句，表有500多W记录，目前已经执行了35秒了。	
					-- 每隔30秒判断一次是否有执行耗时超过10秒的SQL语句。
			之后从库也有长事务，同时从库有延迟现象
		...
		反馈的信息：
			1. 收到长事务报警，执行了35秒，而且还没有执行完成，在数据库做了1个通过delete语句删除全表记录的操作，表有500多W记录
				总耗时约80秒。
			2. 之后从库也有长事务 + 从库有延迟 的报警
	...
	解决办法：
		...
		1. 修改主从复制参数，slave_rows_search_algorithms 为  "INDEX_SCAN,HASH_SCAN"
			...
			在平常的测试中，发现50W行的小表可以用，这种情况下有会10左右的性能提升，之前测试过。
