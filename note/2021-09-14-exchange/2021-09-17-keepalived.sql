...
基本原理：
	...
	通过 keepalived 监控主库 mysqld 的存活，当监控到 mysqld 宕机，自动把主库的keepalived进程杀掉，让VIP进行漂移到从库，这时候从库接管服务，这个主从自动切换的过程不需要人为干预。
...
思考：
	...
	主库所在的机器宕机，会触发MySQL的主从切换吗
	
		答：会的。
	
	为什么MHA也可以做到?
		
		答：因为在各个节点都安装有 MHA 的manager包，manager包就是用来监控是否可以连接上主库。
		
			而 keepalived 只是监控当前节点的mysqld进程是否存在 。
