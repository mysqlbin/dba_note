...
2021-12-13 进行二面：
	...
	1. 数据库上云比较稳定之后，DBA还需要做什么 
		1. 可以考虑做一下容灾之类的
		2. 看看业务有没有一些隐患之类的，提前做下优化，比如慢查询
		3. 往故障自愈方面思考
		
	2. 自建的数据库稳定之后，还需要做什么 或者 你在做什么
		1. 不可能一直稳定的
		2. 跟踪和学习最新版本的新特性，做储备工作
		3. 提前发现一些有隐患的地方
			看看参数有没有设置不合理的，比如 time_zone 参数
		4. 源码的阅读，更深入的理解MySQL，然后运用到工作上。
	...
	5. 自建的有高可用吗
		...
		补充：使用 keepalive ，有发生过故障切换？  有的，上线前演练过，在业务停服更新的时候，有手工切换过，正常切换后，业务可以正常读写。 

	6. 一条语句，第一次查询耗时100ms，第二次查询耗时10ms, 原因
		第二次查询，直接读取了内存数据页，然后返回，所以执行速度快
