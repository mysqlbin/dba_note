...
1. 异步复制存在数据丢失的问题
	...
	如果主库掉电，高可用HA切换到从库后，有些 binlog还来不及发给从库，所以有丢失数据的风险。
	...
	缺点：
		1. 有数据丢失的风险
			...
			故障分析：
				...
				假设主库在写入engine之后宕机，高可用HA切换到从库后，因为binlog未发送给从库，所以会有导致丢失数据的风险。
...
3. after sync(增强半同步复制、loss-less-semi-synchronous-replication)
	...
	故障分析：
		...
		假设主库在写入engine之后宕机，高可用HA切换到从库后，因为binlog已经写入到从库relay log, 因此不会造成数据丢失。
		但是，会造成从库的事务比主库的事务多：
...
4. 小结
	...
	半同步复制的原理，要结合2阶段提交(两阶段提交细化.png)来看。
