...
1. 初始化表结构和数据
2. 制造主从数据不一致
3. 执行pt-table-checksum和pt-table-sync命令
4. 查看执行pt-table-sync命令的general log
	4.1 master
	4.2 slave
...
3. 执行pt-table-checksum和pt-table-sync命令
	...
	shell> sudo  pt-table-checksum --nocheck-replication-filters --no-check-binlog-format  --replicate=consistency_db.checksums h=192.168.1.10,u=pt_user,p='%123456Abc',P=3306 --databases=sbtest
