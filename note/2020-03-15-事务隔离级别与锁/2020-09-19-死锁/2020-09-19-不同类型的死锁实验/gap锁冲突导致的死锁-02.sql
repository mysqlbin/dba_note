...
事务的执行次序
	...
	时刻点 
		SESSION A                       		SESSION B
		begin;									
	T1	delete from t1 where c2=8;
												begin;
	T2											delete from t1 where c2=6;
												(Query OK)
	T3	insert into t1 select 12,5,5,15;
		(Blocked)
	T4											insert into t1 select 15,9,18,18;
...
死锁分析：	
	...
	1). delete from t1 where c2=8;
		...
		因此该SQL语句的加锁范围是  c2: next-key lock: (6,8] + gap lock (8,10) + id: record lock: c1=6;
														   
	2). delete from t1 where c2=6;
		根据原则1，加锁类型是加 next-key lock， 所以加锁的范围是 (3,6]; 
		根据优化2，加锁类型是加 gap lock, 所以加锁的范围是(6,8);
		因此该SQL语句的加锁范围是  c2: next-key lock: (3,6] + gap lock (6,8) + id: record lock: c1=7;
...
时刻点	
		session A                        SESSION B
	T1 	c2: next-key lock: (6,8] + gap lock (8,10) + id: record lock: c1=6;		

	T2   								 c2: next-key lock: (3,6] + gap lock (6,8) + id: record lock: c1=7;			
	
	T3  在等 c2: next-key lock: (3,6]
	
	T4                					在等 c2: gap lock (8,10)                  
