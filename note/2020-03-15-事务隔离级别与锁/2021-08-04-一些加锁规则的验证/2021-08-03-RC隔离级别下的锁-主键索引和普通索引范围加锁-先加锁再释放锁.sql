...
7. 证明RC隔离级别下先加锁再退化释放锁的实验--基于主键索引范围等值查询
	7.1 表结构和数据的初始
	7.2 实验1
	7.3 实验2
	7.5 小结
...
10. 证明RC隔离级别下先加锁再退化释放锁的实验--基于主键索引范围等值查询
	10.1 初始表结构和数据初始化
	10.2 现象和我的问题
	10.3 MySQL 8.0.17版本
...
4.1 实验1--先等值范围查询加锁再等值查询加锁
	...
	session A 的加锁过程:
		...
		RC隔离级别:
			...
			普通索引的范围等值查询, 需要访问到不满足条件的第一个值为止, 并且需要加锁, 不会把不满足条件的锁释放掉.
				...
				如果是更新语句(update/delete), 需要回到主键索引上进行加锁，再释放不满足条件的第一个值的锁。
			...
			如果是主键索引的范围等值查询,也是需要访问到不满足条件的第一个值为止, 并且需要加锁, 判断该记录不符合查询条件，会释放掉这条记录的锁. 
	...
	T1时刻
		...
		| ENGINE_LOCK_ID                        | ENGINE_TRANSACTION_ID | THREAD_ID | OBJECT_NAME | INDEX_NAME | LOCK_TYPE | LOCK_MODE     | LOCK_STATUS | LOCK_DATA    |
		+---------------------------------------+-----------------------+-----------+-------------+------------+-----------+---------------+-------------+--------------+
		| 139835056597608:1062:139834941305512  |                  3673 |        77 | hero        | NULL       | TABLE     | IX            | GRANTED     | NULL         |
		| 139835056597608:5:5:2:139834941302472 |                  3673 |        77 | hero        | idx_name   | RECORD    | X,REC_NOT_GAP | GRANTED     | 'l刘备', 1   |
		| 139835056597608:5:5:4:139834941302472 |                  3673 |        77 | hero        | idx_name   | RECORD    | X,REC_NOT_GAP | GRANTED     | 'c曹操', 8   |
		| 139835056597608:5:4:4:139834941302816 |                  3673 |        77 | hero        | PRIMARY    | RECORD    | X,REC_NOT_GAP | GRANTED     | 8            |
		+---------------------------------------+-----------------------+-----------+-------------+------------+-----------+---------------+-------------+--------------+
	...
	T2时刻
		...
		| ENGINE_LOCK_ID                        | ENGINE_TRANSACTION_ID | THREAD_ID | OBJECT_NAME | INDEX_NAME | LOCK_TYPE | LOCK_MODE     | LOCK_STATUS | LOCK_DATA    |
		+---------------------------------------+-----------------------+-----------+-------------+------------+-----------+---------------+-------------+--------------+
		| 139835056598480:1062:139834941311464  |                  3674 |        76 | hero        | NULL       | TABLE     | IX            | GRANTED     | NULL         |
		| 139835056598480:5:5:2:139834941308536 |                  3674 |        76 | hero        | idx_name   | RECORD    | X,REC_NOT_GAP | WAITING     | 'l刘备', 1   |
...
4.2 实验2--范围更新加锁		
	...
	验证是否会对不满足条件的第一个值对应的主键值进行加锁再释放锁：
...
11. 小结
	...
	6. 虽然做总结会比较耗时一点, 但是为了掌握得更好和复习, 必须是要做总结的.
	
	7. RC隔离级别:
		普通索引的范围等值查询(lock in share mode、for update模式的加锁), 需要访问到不满足条件的第一个值为止, 并且需要加锁, 语句结束不会把不满足条件的锁释放掉.
...
12. 实践指导意义
	...
	少用范围更新，可以先把数据查询出来，再根据主键ID进行等值更新。
