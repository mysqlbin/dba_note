...
原表			原表     	 					原表 
新表    		临时文件     					跟原表一样的表结构，新表以gho结尾
全量数据的处理  拷贝到临时文件                  按一定chunk大小拷贝到新表中               
增量数据的处理	记录到日志文件  				会记录到binlog中
				应用日志文件的数据到临时文件中	备库拉取binlog并在过滤之后应用到原库的新表上。
表名的交换      用临时文件替换表A的数据文件		把原表改为以 del 结尾的表, 新表改为原表，删除以 del 结尾的表    # 原子操作				
...
1. 简介
	2016年8月份,shlomi-noach在GitHub Engineering发文宣布gh-ost开源。
	gh-ost是什么？一个不依赖触发器实现的在线表结构变更
...
3. 其大致的工作过程
	...
	1. gh-ost 首先连接到主库上，根据 alter 语句创建1个跟原表表结构一样的新表，是 gho 结尾的

	2. 然后作为一个备库(类似伪装的备库)连接到其中一个真正的备库或者主库上(根据具体的参数来定)，
...
8. 连接到主库的大致的操作过程如下
	...
	8.6 copy完数据之后进行原始表和影子表cut-over 切换
