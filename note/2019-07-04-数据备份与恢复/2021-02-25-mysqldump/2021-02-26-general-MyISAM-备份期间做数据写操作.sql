...
4. 结论
5. 思考
6. 备份期间做数据删除操作输出的general log 如下	
7. 相关参考

0. 实验目的
	验证备份MyISAM表期间，有DML操作，会发生什么 
...
5. 思考
	...
	答：
		...
		sessionA 备份流程                session B DML操作              
		创建一致性快照
		show master status; 获取位点
										Q1:在备份表前做数据的插入
		备份表数据
										Q2:在备份表后做数据的插入		
										
										
		Q1阶段，在备份表数据前做数据的插入，之后备份表数据，当拿这个备份+binlog做从库的时候，导致从库在应用 relay log做数据写入的时候，报主键冲突；
		Q1阶段，在备份表数据后做数据的插入，之前备份表数据，当拿这个备份+binlog做从库的时候，数据是一致的。
