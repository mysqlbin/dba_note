...
6. 小结
	...
	从库的不可靠，特别是大表，只有 insert 的场景。有读写分离的要注意了，要定期更新从库核心表的统计信息，从而避免因为统计信息不够新，导致SQL选择了错误的索引引发SQL执行速度慢。
	...
	------------------------------------------------------------------------------------------------------------------------------------
	表的数据量大，纯insert场景，因此需要插入记录数相对较多，才会触发自动更新统计信息，因此统计信息相对于当前时间，有一定的落后。
