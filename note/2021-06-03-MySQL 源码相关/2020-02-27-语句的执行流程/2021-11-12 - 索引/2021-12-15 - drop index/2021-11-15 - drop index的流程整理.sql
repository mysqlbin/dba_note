...
3. 删除二级索引会释放(回收)磁盘空间吗
	...
	普通删除索引的话，不会释放(回收)磁盘空间。
	由于索引(二级索引)和数据(主键索引)是存储在同一个文件中，因此在使用独立表空间时，InnoDB引擎使用drop命令删除索引并不会释放(回收)磁盘空间。
	
	
	但是，索引删除后，之前索引占用的表空间(释放索引树占用的表空间)，可以被重用。
...
4. 相关参考
	...
	http://mysql.taobao.org/monthly/2020/01/01/
...
6. 小结与解惑
	...
	删除二级索引，会释放索引树，并不需要扫描二级索引的记录并对其打删除标记，因此执行速度很快。
	
	
title：从源码角度浅析MySQL执行drop index操作速度快的原因
