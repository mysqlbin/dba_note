...
目前这3个表每天的新增数据量大概如下
	table_clublogscore：       40MB
	table_third_score_detail:  40MB
	table_clubgamescoredetail: 60MB

目前这3个表还没有到90天的数据（目前已经有83天的数据），所以没有做数据的删除操作。

直接在线做drop删除表操作，根据drop 删除表的流程，主要的影响：对内存池加锁，导入数据库不可写入和读取。

所以还是觉得在每次停服的时候，再把这3个表90天前的数据，一次性删除掉会更好，不会对业务有影响。
