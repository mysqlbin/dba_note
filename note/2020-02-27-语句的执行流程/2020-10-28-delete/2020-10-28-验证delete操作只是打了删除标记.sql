...
3. 小结
	...
	delete语句只是对行记录打了一个删除标记，后台purge线程真正删除这些记录(一次性删除的记录数多，此时的IO利用率相对较高，所以还是要把大事务拆分为小事务)，释放物理页空间
	-- 删除数据的大事务，也就是一次性删除多行记录，会导致IO利用率高。
	后期学习源码，再详细看......  --参考笔记：《2021-06-22-delete语句的函数栈.sql》
