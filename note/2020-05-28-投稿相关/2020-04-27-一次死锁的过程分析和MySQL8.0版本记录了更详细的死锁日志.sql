...
1. 环境
	事务隔离级别为RC读已提交
	MySQL版本为 5.7.26
	先介绍一下数据表情况，因为涉及到公司内部真实的数据，所以以下都做了模拟，不会影响具体的分析。
...
3. 死锁日志分析
	...
	3.2 声明
		死锁日志中的 事务一 为： TRANSACTION 18912129
		死锁日志中的 事务二 为： TRANSACTION 18912896 
	...
	3.5 根据死锁信息分析出的两个事务的加锁规则和死锁成因
		...
		T1: 持有主键索引 ID=1 的记录锁, 锁的模式为排他锁

											  	 T2: 持有辅助索引idx_status_createtime: record lock: 
											   		status=0, createtime='2020-04-24 12:10:00', ID=1) 的记录锁，锁的模式为排他锁
											   
											  	 T3: 在等主键索引 ID=1 的记录锁	

		T4: 在等辅助索引idx_status_createtime: 
			record lock: (status=0, createtime='2020-04-24 12:10:00'，ID=1)的记录锁
			
				 
		T3被T1阻塞，T4被T2阻塞，因此锁资源请求形成了环路，进而触发死锁检测, 因此导致了死锁。	


4. 根据上面的死锁分析和业务逻辑复现该死锁是如何形成的
	事务一                                           事务二                                  		事务三
	
	begin; 
	select status from t1 where 
		order_no='123456' for update; 
	T1: 持有的锁： 
    	idx_order_no：record lock：
    		(order_no='123456', ID=1)
    	PRIMARY：record lock：[1]   

													UPDATE t1 SET status = 5 WHERE
													 status = 0 AND (`createtime` BETWEEN DATE_SUB(NOW(),INTERVAL 90 MINUTE) 
													 	AND DATE_SUB(NOW(),INTERVAL 60 MINUTE));
													(Blocked, 被阻塞)
													T2: 持有的锁：
      													idx_status_createtime: record lock： 
      													（status=0, createtime='2020-04-24 12:10:00', ID=1）
													T3: 在等待的锁： PRIMARY: record lock：[1]
	...
	查看 information_schema 下的两个视图 innodb_locks 和 innodb_lock_waits 确认持有的锁和在等待的锁的详情																			
		...
		*************************** 2. row ***************************
		...
		事务二更新语句的WHERE条件是查询到的记录是为空的, 但是为什么会持有辅助索引idx_status_createtime上面的锁呢？
		...
		MySQL8.0 版本information_schema中的innodb_locks和innodb_lock_waits表被移除，取而代之的是performance_schema中的data_locks和data_lock_waits表，通过 data_locks 可以看到锁数据的详情。
...
5. 解决本案例死锁的办法之一
	经过跟开发人员沟通, 事务一的 UPDATE语句可以不放在显示声明的事务内执行, 然后把更新语句移出显示声明的事务中就可以避免本案例的死锁。
...
6. MySQL 8.0.19 版本测试本案例
	...
	事务的执行顺序
		...
		查看 information_schema.innodb_locks 确认事务持有的锁和在等待的锁的详情															
			...
			INDEX_NAME=PRIMARY, LOCK_TYPE=RECORD, LOCK_MODE='X,REC_NOT_GAP', LOCK_STATUS=WAITING, LOCK_DATA=1 表示 事务二在等的锁为: PRIMARY: record lock: [1]
	...
	死锁日志
		...
		*** (1) HOLDS THE LOCK(S):             --持有的锁
		...
		Record lock, heap no 2 PHYSICAL RECORD: n_fields 3; compact format; info bits 0
		 0: len 4; hex 80000000; asc     ;;    --status 字段, 值为 0
		 1: len 4; hex 5ea26698; asc ^ f ;;    --createtime字段, 值为 '2020-04-24 12:10:00'
		 2: len 4; hex 00000001; asc     ;;    --主键ID字段,  值为 1
		...
		*** (1) WAITING FOR THIS LOCK TO BE GRANTED:  --在等待的锁
		...
		Record lock, heap no 2 PHYSICAL RECORD: n_fields 8; compact format; info bits 0
		 0: len 4; hex 00000001; asc     ;;           --主键ID字段,  值为 1
		 1: len 6; hex 000000033c54; asc     <T;;
		 2: len 7; hex 0200000ff80e6a; asc       j;;
		 3: len 4; hex 80000001; asc     ;;
		 4: len 4; hex 80000001; asc     ;;
		 5: len 6; hex 313233343536; asc 123456;;     --order_no 字段, 值为 123456
		 6: len 4; hex 80000001; asc     ;;
		 7: len 4; hex 5ea26698; asc ^ f ;;           --createtime字段, 值为 '2020-04-24 12:10:00'
	...
	分析死锁日志
		...
		TRANSACTION 212052 记为事务一
		TRANSACTION 212055 记为事务二
		分析死锁的时候，我习惯把事务编号小的定义为事务一
		...
		事务一的信息
			...
			在等待的锁信息
				Record lock  --是一个记录锁
				index idx_status_createtime of table `sbtest`.`t1`  --表示在等的是表t1 的辅助索引 idx_status_createtime 上面的锁
				lock_mode X locks rec but not gap waiting           --表示需要加一个排他锁（写锁），当前的状态是等待中
				
				n_fields 3: --表示辅助索引idx_status_createtime的记录是3列, 根据索引的存储结构, 这3列依次为 status、createtime、ID

				0: len 4; hex 80000000; asc     ;;   --表示 status字段, 值为 0
				1: len 4; hex 5ea26698; asc ^ f ;;   --表示 createtime字段, 值为 2020-04-24 12:10:00
				2: len 4; hex 00000001; asc     ;;   --表示 ID字段, 值为1
				
				所以在等待事务二 (status=0,createtime='2020-04-24 12:10:00',ID=1) 这一行的记录锁
		...
		事务二的信息
			...
			持有的锁信息
				...
				HOLDS THE LOCK(S)                     --用来表示这个事务持有哪些锁
				index idx_status_createtime of table `sbtest`.`t1`  --表示加锁是在加在表t1 的辅助索引idx_status_createtime 上
				n_fields 3:                           --表示这个记录是3列
				 0: len 4; hex 80000000; asc     ;;    --status 字段, 值为 0
				 1: len 4; hex 5ea26698; asc ^ f ;;    --时间createtime字段, 值为  '2020-04-24 12:10:00'
				 2: len 4; hex 00000001; asc     ;;    --主键ID字段,  值为 1
			...
			在等待的锁信息
				...
				index PRIMARY of table `sbtest`.`t1`          --表示在等的是表 t1 的主键索引PRIMARY 上面的锁
				lock_mode X locks rec but not gap waiting   --表示需要加一个排他锁（写锁），当前的状态是等待中
				Record lock  						  --表示是一个记录锁
				n_fields 8:                           --表示这个记录是8列
				0: len 4; hex 00000001; asc     ;;    --表示第一个列，也就是主键字段 ID, 值为 1
					
				所以在等待事务一的: record lock ：(ID=1)。
	...
	加锁的详情如下
		...
		持有的锁: 
			idx_order_no：record lock：
    		 (order_no='123456', ID=1) 和 
    		PRIMARY： record lock：[1]  

												持有的锁:   
													idx_status_createtime: record lock： 
      												 (status=0, createtime='2020-04-24 12:10:00', ID=1)
												在等待的锁:	PRIMARY: record lock: [1]
...
7. 小结
	死锁一般可以结合死锁日志、加锁规则和业务场景来做相关的分析, 并能模拟出来
	辅助索引的范围查询更新加锁，需要往后找到一条满足条件的记录才会停止扫描
	加锁是在加在索引上的，当表中有多个索引，只会对必要的索引加锁，例如本案例中的表t1有3个索引，分别为 PRIMARY KEY、idx_order_no和idx_status_createtime
	 当执行 select * from t1 where order_no='123456' for update; 语句后，会对索引 PRIMARY KEY、idx_order_no 的记录进行加锁，并不会对索引 idx_status_createtime 的记录进行加锁。
	MySQL 8.0版本通过 performance_schema.data_locks 可以看到事务锁定的更详细的信息，也可以看到辅助索引锁住的数据。
