...
2. 使用建议
	...
	1. 设置 innodb_temp_data_file_path 选项，设定文件最大上限，超过上限时，需要生成临时表的SQL无法被执行（一般这种SQL效率也比较低，可借此机会进行优化）。
		...
		可以验证下。
		验证了，这并不是一个好的idea。监控长事务可以从根本上解决问题。
...
4. 临时表初体验 
	...
	*************************** 1. row ***************************
	...
	查看临时表元数据信息
		...
		root@mysqldb 17:32:  [yldbs]> select * from INFORMATION_SCHEMA.INNODB_TEMP_TABLE_INFO\G;
		*************************** 1. row ***************************
					TABLE_ID: 98560
						NAME: #sql180c_4_0    --表名
					  N_COLS: 6
...
6. 相关思考
	1. 临时表跟普通表的区别
		临时表：
			连接断开、数据库重启，临时表会被自动删除。
			表名的定义: 主要由进程号+线程号+序列号
	...
	3. 不同线程可以使用同名的临时表，这个没有问题。但是如果在程序中，用的是连接池中的连接来操作的，而这些连接不会释放，和数据库保持长连接。这样使用临时表会有问题吗?
	
		会，”临时表会自动回收”这个功能，主要用于“应用程序异常断开、MySQL异常重启”后，不需要主动去删除表。

		而平时正常使用的时候，用完删除，还是应该有的好习惯。	
		
		-- 同1个线程/连接对象不可以创建相同的临时表。
...
8. 验证临时表是session级别的
	...
	/home/mysql/mysql3306/tmp
	[root@localhost tmp]# ll
	总用量 24
	-rw-r----- 1 mysql mysql 8804 4月  11 13:03 #sqledf_7d7_0.frm
	-rw-r----- 1 mysql mysql 8804 4月  11 13:03 #sqledf_7d8_0.frm
